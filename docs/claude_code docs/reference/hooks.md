...
The table below summarizes when each event fires. The [Hook events](https://docs.anthropic.com/en/docs/claude-code/hooks#hook-events) section documents the full input schema and decision control options for each one.
...
The [Configuration](https://docs.anthropic.com/en/docs/claude-code/hooks#configuration) section below documents the full schema, and each [hook event](https://docs.anthropic.com/en/docs/claude-code/hooks#hook-events) section documents what input your command receives and what output it can return.

## Configuration

Hooks are defined in JSON settings files. The configuration has three levels of nesting:

1.   Choose a [hook event](https://docs.anthropic.com/en/docs/claude-code/hooks#hook-events) to respond to, like `PreToolUse` or `Stop`
2.   Add a [matcher group](https://docs.anthropic.com/en/docs/claude-code/hooks#matcher-patterns) to filter when it fires, like “only for the Bash tool”
3.   Define one or more [hook handlers](https://docs.anthropic.com/en/docs/claude-code/hooks#hook-handler-fields) to run when matched

See [How a hook resolves](https://docs.anthropic.com/en/docs/claude-code/hooks#how-a-hook-resolves) above for a complete walkthrough with an annotated example.
...
The `FileChanged` event does not follow these rules when building its watch list. See [FileChanged](https://docs.anthropic.com/en/docs/claude-code/hooks#filechanged).Each event type matches on a different field:
...
| `FileChanged` | literal filenames to watch (see [FileChanged](https://docs.anthropic.com/en/docs/claude-code/hooks#filechanged)) | `.envrc|.env` |
...
The matcher runs against a field from the [JSON input](https://docs.anthropic.com/en/docs/claude-code/hooks#hook-input-and-output) that Claude Code sends to your hook on stdin. For tool events, that field is `tool_name`. Each [hook event](https://docs.anthropic.com/en/docs/claude-code/hooks#hook-events) section lists the full set of matcher values and the input schema for that event.This example runs a linting script only when Claude writes or edits a file:
...
`UserPromptSubmit`, `PostToolBatch`, `Stop`, `TeammateIdle`, `TaskCreated`, `TaskCompleted`, `WorktreeCreate`, `WorktreeRemove`, and `CwdChanged` don’t support matchers and always fire on every occurrence. If you add a `matcher` field to these events, it is silently ignored.For tool events, you can filter more narrowly by setting the [`if` field](https://docs.anthropic.com/en/docs/claude-code/hooks#common-fields) on individual hook handlers. `if` uses [permission rule syntax](https://code.claude.com/docs/en/permissions) to match against the tool name and arguments together, so `"Bash(git *)"` runs when any subcommand of the Bash input matches `git *` and `"Edit(*.ts)"` runs only for TypeScript files.
...
*   **[Command hooks](https://docs.anthropic.com/en/docs/claude-code/hooks#command-hook-fields)** (`type: "command"`): run a shell command. Your script receives the event’s [JSON input](https://docs.anthropic.com/en/docs/claude-code/hooks#hook-input-and-output) on stdin and communicates results back through exit codes and stdout.
*   **[HTTP hooks](https://docs.anthropic.com/en/docs/claude-code/hooks#http-hook-fields)** (`type: "http"`): send the event’s JSON input as an HTTP POST request to a URL. The endpoint communicates results back through the response body using the same [JSON output format](https://docs.anthropic.com/en/docs/claude-code/hooks#json-output) as command hooks.
*   **[MCP tool hooks](https://docs.anthropic.com/en/docs/claude-code/hooks#mcp-tool-hook-fields)** (`type: "mcp_tool"`): call a tool on an already-connected [MCP server](https://code.claude.com/docs/en/mcp). The tool’s text output is treated like command-hook stdout.
*   **[Prompt hooks](https://docs.anthropic.com/en/docs/claude-code/hooks#prompt-and-agent-hook-fields)** (`type: "prompt"`): send a prompt to a Claude model for single-turn evaluation. The model returns a yes/no decision as JSON. See [Prompt-based hooks](https://docs.anthropic.com/en/docs/claude-code/hooks#prompt-based-hooks).
*   **[Agent hooks](https://docs.anthropic.com/en/docs/claude-code/hooks#prompt-and-agent-hook-fields)** (`type: "agent"`): spawn a subagent that can use tools like Read, Grep, and Glob to verify conditions before returning a decision. Agent hooks are experimental and may change. See [Agent-based hooks](https://docs.anthropic.com/en/docs/claude-code/hooks#agent-based-hooks).
...
| `if` | no | Permission rule syntax to filter when this hook runs, such as `"Bash(git *)"` or `"Edit(*.ts)"`. The hook command only runs if the tool call matches the pattern. See the [Bash matching table](https://docs.anthropic.com/en/docs/claude-code/hooks#bash-if-matching) below for how Bash patterns evaluate against subcommands, `$()`, and backticks. Only evaluated on tool events: `PreToolUse`, `PostToolUse`, `PostToolUseFailure`, `PermissionRequest`, and `PermissionDenied`. On other events, a hook with `if` set never runs. Uses the same syntax as [permission rules](https://code.claude.com/docs/en/permissions) |
| `timeout` | no | Seconds before canceling. Defaults: 600 for `command`, `http`, and `mcp_tool`; 30 for `prompt`; 60 for `agent`. [`UserPromptSubmit`](https://docs.anthropic.com/en/docs/claude-code/hooks#userpromptsubmit) lowers the `command`, `http`, and `mcp_tool` default to 30, and [`MessageDisplay`](https://docs.anthropic.com/en/docs/claude-code/hooks#messagedisplay) lowers it to 10 |
| `statusMessage` | no | Custom spinner message displayed while the hook runs |
| `once` | no | If `true`, runs once per session then is removed. Only honored for hooks declared in [skill frontmatter](https://docs.anthropic.com/en/docs/claude-code/hooks#hooks-in-skills-and-agents); ignored in settings files and agent frontmatter |
...
In addition to the [common fields](https://docs.anthropic.com/en/docs/claude-code/hooks#common-fields), command hooks accept these fields:

| Field | Required | Description |
| --- | --- | --- |
| `command` | yes | Shell command to execute. With `args`, the executable to spawn directly. See [Exec form and shell form](https://docs.anthropic.com/en/docs/claude-code/hooks#exec-form-and-shell-form) |
| `args` | no | Argument list. When present, `command` is resolved as an executable and spawned directly with `args` as the argument vector, with no shell involved. See [Exec form and shell form](https://docs.anthropic.com/en/docs/claude-code/hooks#exec-form-and-shell-form) |
| `async` | no | If `true`, runs in the background without blocking. See [Run hooks in the background](https://docs.anthropic.com/en/docs/claude-code/hooks#run-hooks-in-the-background) |
...
Both forms support the same [path placeholders](https://docs.anthropic.com/en/docs/claude-code/hooks#reference-scripts-by-path), and both export them as the environment variables `CLAUDE_PROJECT_DIR`, `CLAUDE_PLUGIN_ROOT`, and `CLAUDE_PLUGIN_DATA` on the spawned process, so a script can read `process.env.CLAUDE_PLUGIN_ROOT` regardless of how it was launched. Plugin hooks additionally substitute `${user_config.*}` values; see [User configuration](https://code.claude.com/docs/en/plugins-reference#user-configuration).

#### HTTP hook fields

In addition to the [common fields](https://docs.anthropic.com/en/docs/claude-code/hooks#common-fields), HTTP hooks accept these fields:
...
Claude Code sends the hook’s [JSON input](https://docs.anthropic.com/en/docs/claude-code/hooks#hook-input-and-output) as the POST request body with `Content-Type: application/json`. The response body uses the same [JSON output format](https://docs.anthropic.com/en/docs/claude-code/hooks#json-output) as command hooks.Error handling differs from command hooks: non-2xx responses, connection failures, and timeouts all produce non-blocking errors that allow execution to continue. To block a tool call or deny a permission, return a 2xx response with a JSON body containing `decision: "block"` or a `hookSpecificOutput` with `permissionDecision: "deny"`.This example sends `PreToolUse` events to a local validation service, authenticating with a token from the `MY_TOKEN` environment variable:
...
In addition to the [common fields](https://docs.anthropic.com/en/docs/claude-code/hooks#common-fields), MCP tool hooks accept these fields:
...
In addition to the [common fields](https://docs.anthropic.com/en/docs/claude-code/hooks#common-fields), prompt and agent hooks accept these fields:
...
Command hooks receive JSON data via stdin and communicate results through exit codes, stdout, and stderr. HTTP hooks receive the same JSON as the POST request body and communicate results through the HTTP response body. This section covers fields and behavior common to all events. Each event’s section under [Hook events](https://docs.anthropic.com/en/docs/claude-code/hooks#hook-events) includes its specific input schema and decision control options.On macOS and Linux, command hooks run in their own session without a controlling terminal as of v2.1.139. The hook process and any child processes cannot open `/dev/tty` or send escape sequences directly to the Claude Code interface. Windows has no `/dev/tty`. To surface a message to the user on any platform, return [`systemMessage`](https://docs.anthropic.com/en/docs/claude-code/hooks#json-output) in JSON output. To trigger a desktop notification, set a window title, or ring the bell, return [`terminalSequence`](https://docs.anthropic.com/en/docs/claude-code/hooks#emit-terminal-notifications) instead.

### Common input fields

Hook events receive these fields as JSON, in addition to event-specific fields documented in each [hook event](https://docs.anthropic.com/en/docs/claude-code/hooks#hook-events) section. For command hooks, this JSON arrives via stdin. For HTTP hooks, it arrives as the POST request body.
...
The `tool_name` and `tool_input` fields are event-specific. Each [hook event](https://docs.anthropic.com/en/docs/claude-code/hooks#hook-events) section documents the additional fields for that event.

### Exit code output

The exit code from your hook command tells Claude Code whether the action should proceed, be blocked, or be ignored.**Exit 0** means success. Claude Code parses stdout for [JSON output fields](https://docs.anthropic.com/en/docs/claude-code/hooks#json-output). JSON output is only processed on exit 0. For most events, stdout is written to the debug log but not shown in the transcript. The exceptions are `UserPromptSubmit`, `UserPromptExpansion`, and `SessionStart`, where stdout is added as context that Claude can see and act on.**Exit 2** means a blocking error. Claude Code ignores stdout and any JSON in it. Instead, stderr text is fed back to Claude as an error message. The effect depends on the event: `PreToolUse` blocks the tool call, `UserPromptSubmit` rejects the prompt, and so on. See [exit code 2 behavior](https://docs.anthropic.com/en/docs/claude-code/hooks#exit-code-2-behavior-per-event) for the full list.**Any other exit code** is a non-blocking error for most hook events. The transcript shows a `<hook name> hook error` notice followed by the first line of stderr, so you can identify the cause without `--debug`. Execution continues and the full stderr is written to the debug log.For example, a hook command script that blocks dangerous Bash commands:
...
*   **2xx with a JSON body**: success, parsed using the same [JSON output](https://docs.anthropic.com/en/docs/claude-code/hooks#json-output) schema as command hooks
...
*   [SessionStart](https://docs.anthropic.com/en/docs/claude-code/hooks#sessionstart), [Setup](https://docs.anthropic.com/en/docs/claude-code/hooks#setup), and [SubagentStart](https://docs.anthropic.com/en/docs/claude-code/hooks#subagentstart): at the start of the conversation, before the first prompt
*   [UserPromptSubmit](https://docs.anthropic.com/en/docs/claude-code/hooks#userpromptsubmit) and [UserPromptExpansion](https://docs.anthropic.com/en/docs/claude-code/hooks#userpromptexpansion): alongside the submitted prompt
*   [PreToolUse](https://docs.anthropic.com/en/docs/claude-code/hooks#pretooluse), [PostToolUse](https://docs.anthropic.com/en/docs/claude-code/hooks#posttooluse), [PostToolUseFailure](https://docs.anthropic.com/en/docs/claude-code/hooks#posttoolusefailure), and [PostToolBatch](https://docs.anthropic.com/en/docs/claude-code/hooks#posttoolbatch): next to the tool result
*   [Stop](https://docs.anthropic.com/en/docs/claude-code/hooks#stop) and [SubagentStop](https://docs.anthropic.com/en/docs/claude-code/hooks#subagentstop): at the end of the turn. The conversation continues so Claude can act on the feedback. See [Stop decision control](https://docs.anthropic.com/en/docs/claude-code/hooks#stop-decision-control)
...
| --- | --- | --- |
| UserPromptSubmit, UserPromptExpansion, PostToolUse, PostToolUseFailure, PostToolBatch, Stop, SubagentStop, ConfigChange, PreCompact | Top-level `decision` | `decision: "block"`, `reason`. Stop and SubagentStop also accept `hookSpecificOutput.additionalContext` for [non-error feedback that continues the conversation](https://docs.anthropic.com/en/docs/claude-code/hooks#stop-decision-control) |
...
| SessionStart, Setup, SubagentStart | Context only | `hookSpecificOutput.additionalContext` adds context for Claude. SessionStart also accepts [`initialUserMessage`, `watchPaths`, `sessionTitle`, and `reloadSkills`](https://docs.anthropic.com/en/docs/claude-code/hooks#sessionstart-decision-control). No blocking or decision control |
...
*   `PreToolUse` — `updatedInput` directly under `hookSpecificOutput` replaces a tool’s arguments before it runs ([details](https://docs.anthropic.com/en/docs/claude-code/hooks#pretooluse-decision-control))
*   `PermissionRequest` — `updatedInput` inside the `decision` object ([details](https://docs.anthropic.com/en/docs/claude-code/hooks#permissionrequest-decision-control))
*   `PostToolUse` — `updatedToolOutput` replaces the tool’s result ([details](https://docs.anthropic.com/en/docs/claude-code/hooks#posttooluse-decision-control))
...
In addition to the [common input fields](https://docs.anthropic.com/en/docs/claude-code/hooks#common-input-fields), SessionStart hooks receive `source` and optionally `model`, `agent_type`, and `session_title`. The `source` field indicates how the session started: `"startup"` for new sessions, `"resume"` for resumed sessions, `"clear"` after `/clear`, or `"compact"` after compaction. The `model` field contains the active model identifier. It can be omitted, for example after `/clear` or when a session is restored through conversation recovery, so check for the field before reading it. If you start Claude Code with `claude --agent <name>`, an `agent_type` field contains the agent name. The `session_title` field carries the current session title if one is already set, for example via `--name` or `/rename`. A hook that emits `sessionTitle` can check `session_title` first to avoid overwriting a title the user set explicitly.
...
Any text your hook script prints to stdout is added as context for Claude. In addition to the [JSON output fields](https://docs.anthropic.com/en/docs/claude-code/hooks#json-output) available to all hooks, you can return these event-specific fields:

| Field | Description |
| --- | --- |
| `additionalContext` | String added to Claude’s context at the start of the conversation, before the first prompt. See [Add context for Claude](https://docs.anthropic.com/en/docs/claude-code/hooks#add-context-for-claude) for how the text is delivered and what to put in it |
...
| `watchPaths` | Array of absolute paths to watch for [FileChanged](https://docs.anthropic.com/en/docs/claude-code/hooks#filechanged) events during this session |
...
In addition to the [common input fields](https://docs.anthropic.com/en/docs/claude-code/hooks#common-input-fields), Setup hooks receive a `trigger` field set to either `"init"` or `"maintenance"`:
...
Setup hooks cannot block. On exit code 2, stderr is shown to the user; on any other non-zero exit code, stderr appears only when you launch with `--verbose`. In both cases execution continues. To pass information into Claude’s context, return `additionalContext` in JSON output; plain stdout is written to the debug log only. In addition to the [JSON output fields](https://docs.anthropic.com/en/docs/claude-code/hooks#json-output) available to all hooks, you can return these event-specific fields:
...
Setup hooks have access to `CLAUDE_ENV_FILE`. Variables written to that file persist into subsequent Bash commands for the session, just as in [SessionStart hooks](https://docs.anthropic.com/en/docs/claude-code/hooks#persist-environment-variables). Only `type: "command"` and `type: "mcp_tool"` hooks are supported.
...
In addition to the [common input fields](https://docs.anthropic.com/en/docs/claude-code/hooks#common-input-fields), InstructionsLoaded hooks receive these fields:
...
In addition to the [common input fields](https://docs.anthropic.com/en/docs/claude-code/hooks#common-input-fields), UserPromptSubmit hooks receive the `prompt` field containing the text the user submitted.
...
`UserPromptSubmit` hooks can control whether a user prompt is processed and add context. All [JSON output fields](https://docs.anthropic.com/en/docs/claude-code/hooks#json-output) are available.There are two ways to add context to the conversation on exit code 0:
...
| `additionalContext` | String added to Claude’s context alongside the submitted prompt. See [Add context for Claude](https://docs.anthropic.com/en/docs/claude-code/hooks#add-context-for-claude) |
...
In addition to the [common input fields](https://docs.anthropic.com/en/docs/claude-code/hooks#common-input-fields), UserPromptExpansion hooks receive `expansion_type`, `command_name`, `command_args`, `command_source`, and the original `prompt` string. The `expansion_type` field is `slash_command` for skill and custom commands, or `mcp_prompt` for MCP server prompts.
...
`UserPromptExpansion` hooks can block the expansion or add context. All [JSON output fields](https://docs.anthropic.com/en/docs/claude-code/hooks#json-output) are available.
...
| `additionalContext` | String added to Claude’s context alongside the expanded prompt. See [Add context for Claude](https://docs.anthropic.com/en/docs/claude-code/hooks#add-context-for-claude) |
...
In addition to the [common input fields](https://docs.anthropic.com/en/docs/claude-code/hooks#common-input-fields), MessageDisplay hooks receive identifiers for the turn and message, the position of this call within the message, and the new text in `delta`. Batch boundaries depend on how the text streams, so use `index` and `final` to track progress through a message rather than expecting lines to be grouped a particular way.
...
In addition to the [JSON output fields](https://docs.anthropic.com/en/docs/claude-code/hooks#json-output) available to all hooks, MessageDisplay hooks can return `displayContent` to replace the delta on screen:
...
Batches with no markdown pass through unchanged. If the script fails, for example because `jq` is missing, Claude Code displays the original text and notes the failure only in [debug output](https://docs.anthropic.com/en/docs/claude-code/hooks#debug-hooks), not in the session.

### PreToolUse

Runs after Claude creates tool parameters and before processing the tool call. Matches on tool name: `Bash`, `Edit`, `Write`, `Read`, `Glob`, `Grep`, `Agent`, `WebFetch`, `WebSearch`, `AskUserQuestion`, `ExitPlanMode`, and any [MCP tool names](https://docs.anthropic.com/en/docs/claude-code/hooks#match-mcp-tools).Use [PreToolUse decision control](https://docs.anthropic.com/en/docs/claude-code/hooks#pretooluse-decision-control) to allow, deny, ask, or defer the tool call.

#### PreToolUse input

In addition to the [common input fields](https://docs.anthropic.com/en/docs/claude-code/hooks#common-input-fields), PreToolUse hooks receive `tool_name`, `tool_input`, and `tool_use_id`. The `tool_input` fields depend on the tool:
...
| `additionalContext` | String added to Claude’s context alongside the tool result. Ignored when `permissionDecision` is `"defer"`. See [Add context for Claude](https://docs.anthropic.com/en/docs/claude-code/hooks#add-context-for-claude) |
...
Runs when the user is shown a permission dialog. Use [PermissionRequest decision control](https://docs.anthropic.com/en/docs/claude-code/hooks#permissionrequest-decision-control) to allow or deny on behalf of the user.Matches on tool name, same values as PreToolUse.
...
`PermissionRequest` hooks can allow or deny permission requests. In addition to the [JSON output fields](https://docs.anthropic.com/en/docs/claude-code/hooks#json-output) available to all hooks, your hook script can return a `decision` object with these event-specific fields:
...
| `updatedPermissions` | For `"allow"` only: array of [permission update entries](https://docs.anthropic.com/en/docs/claude-code/hooks#permission-update-entries) to apply, such as adding an allow rule or changing the session permission mode |
...
The `updatedPermissions` output field and the [`permission_suggestions` input field](https://docs.anthropic.com/en/docs/claude-code/hooks#permissionrequest-input) both use the same array of entry objects. Each entry has a `type` that determines its other fields, and a `destination` that controls where the change is written.
...
`PostToolUse` hooks can provide feedback to Claude after tool execution. In addition to the [JSON output fields](https://docs.anthropic.com/en/docs/claude-code/hooks#json-output) available to all hooks, your hook script can return these event-specific fields:
...
| `additionalContext` | String added to Claude’s context alongside the tool result. See [Add context for Claude](https://docs.anthropic.com/en/docs/claude-code/hooks#add-context-for-claude) |
...
| `updatedMCPToolOutput` | Replaces the output for [MCP tools](https://docs.anthropic.com/en/docs/claude-code/hooks#match-mcp-tools) only. Prefer `updatedToolOutput`, which works for all tools |
...
`PostToolUseFailure` hooks can provide context to Claude after a tool failure. In addition to the [JSON output fields](https://docs.anthropic.com/en/docs/claude-code/hooks#json-output) available to all hooks, your hook script can return these event-specific fields:

| Field | Description |
| --- | --- |
| `additionalContext` | String added to Claude’s context alongside the error. See [Add context for Claude](https://docs.anthropic.com/en/docs/claude-code/hooks#add-context-for-claude) |
...
In addition to the [common input fields](https://docs.anthropic.com/en/docs/claude-code/hooks#common-input-fields), PostToolBatch hooks receive `tool_calls`, an array describing every tool call in the batch:
...
`PostToolBatch` hooks can inject context for Claude. In addition to the [JSON output fields](https://docs.anthropic.com/en/docs/claude-code/hooks#json-output) available to all hooks, your hook script can return these event-specific fields:

| Field | Description |
| --- | --- |
| `additionalContext` | Context string injected once before the next model call. See [Add context for Claude](https://docs.anthropic.com/en/docs/claude-code/hooks#add-context-for-claude) for delivery details, what to put in it, and how resumed sessions handle past values |
...
In addition to the [common input fields](https://docs.anthropic.com/en/docs/claude-code/hooks#common-input-fields), PermissionDenied hooks receive `tool_name`, `tool_input`, `tool_use_id`, and `reason`.
...
In addition to the [common input fields](https://docs.anthropic.com/en/docs/claude-code/hooks#common-input-fields), Notification hooks receive `message` with the notification text, an optional `title`, and `notification_type` indicating which type fired.
...
Notification hooks cannot block or modify notifications. They are intended for side effects such as forwarding the notification to an external service. The [common JSON output fields](https://docs.anthropic.com/en/docs/claude-code/hooks#json-output) such as `systemMessage` apply.
...
In addition to the [common input fields](https://docs.anthropic.com/en/docs/claude-code/hooks#common-input-fields), SubagentStart hooks receive `agent_id` with the unique identifier for the subagent and `agent_type` with the agent name (built-in agents like `"general-purpose"`, `"Explore"`, `"Plan"`, or custom agent names).
...
SubagentStart hooks cannot block subagent creation, but they can inject context into the subagent. In addition to the [JSON output fields](https://docs.anthropic.com/en/docs/claude-code/hooks#json-output) available to all hooks, you can return:

| Field | Description |
| --- | --- |
| `additionalContext` | String added to the subagent’s context at the start of its conversation, before its first prompt. See [Add context for Claude](https://docs.anthropic.com/en/docs/claude-code/hooks#add-context-for-claude) |
...
In addition to the [common input fields](https://docs.anthropic.com/en/docs/claude-code/hooks#common-input-fields), TaskCreated hooks receive `task_id`, `task_subject`, and optionally `task_description`, `teammate_name`, and `team_name`.
...
In addition to the [common input fields](https://docs.anthropic.com/en/docs/claude-code/hooks#common-input-fields), TaskCompleted hooks receive `task_id`, `task_subject`, and optionally `task_description`, `teammate_name`, and `team_name`.
...
`Stop` and `SubagentStop` hooks can control whether Claude continues. In addition to the [JSON output fields](https://docs.anthropic.com/en/docs/claude-code/hooks#json-output) available to all hooks, your hook script can return these event-specific fields:
...
Runs instead of [Stop](https://docs.anthropic.com/en/docs/claude-code/hooks#stop) when the turn ends due to an API error. Output and exit code are ignored. Use this to log failures, send alerts, or take recovery actions when Claude cannot complete a response due to rate limits, authentication problems, or other API errors.

#### StopFailure input

In addition to the [common input fields](https://docs.anthropic.com/en/docs/claude-code/hooks#common-input-fields), StopFailure hooks receive `error`, optional `error_details`, and optional `last_assistant_message`. The `error` field identifies the error type and is used for matcher filtering.
...
In addition to the [common input fields](https://docs.anthropic.com/en/docs/claude-code/hooks#common-input-fields), TeammateIdle hooks receive `teammate_name` and `team_name`.
...
In addition to the [common input fields](https://docs.anthropic.com/en/docs/claude-code/hooks#common-input-fields), ConfigChange hooks receive `source` and optionally `file_path`. The `source` field indicates which configuration type changed, and `file_path` provides the path to the specific file that was modified.
...
Runs when the working directory changes during a session, for example when Claude executes a `cd` command. Use this to react to directory changes: reload environment variables, activate project-specific toolchains, or run setup scripts automatically. Pairs with [FileChanged](https://docs.anthropic.com/en/docs/claude-code/hooks#filechanged) for tools like [direnv](https://direnv.net/) that manage per-directory environment.CwdChanged hooks have access to `CLAUDE_ENV_FILE`. Variables written to that file persist into subsequent Bash commands for the session, just as in [SessionStart hooks](https://docs.anthropic.com/en/docs/claude-code/hooks#persist-environment-variables).CwdChanged does not support matchers and fires on every directory change.

#### CwdChanged input

In addition to the [common input fields](https://docs.anthropic.com/en/docs/claude-code/hooks#common-input-fields), CwdChanged hooks receive `old_cwd` and `new_cwd`.
...
In addition to the [JSON output fields](https://docs.anthropic.com/en/docs/claude-code/hooks#json-output) available to all hooks, CwdChanged hooks can return `watchPaths` to dynamically set which file paths [FileChanged](https://docs.anthropic.com/en/docs/claude-code/hooks#filechanged) watches:
...
*   **Filter which hooks run**: when a watched file changes, the same value filters which hook groups run using the standard [matcher rules](https://docs.anthropic.com/en/docs/claude-code/hooks#matcher-patterns) against the changed file’s basename.

FileChanged hooks have access to `CLAUDE_ENV_FILE`. Variables written to that file persist into subsequent Bash commands for the session, just as in [SessionStart hooks](https://docs.anthropic.com/en/docs/claude-code/hooks#persist-environment-variables).

#### FileChanged input

In addition to the [common input fields](https://docs.anthropic.com/en/docs/claude-code/hooks#common-input-fields), FileChanged hooks receive `file_path` and `event`.
...
In addition to the [JSON output fields](https://docs.anthropic.com/en/docs/claude-code/hooks#json-output) available to all hooks, FileChanged hooks can return `watchPaths` to dynamically update which file paths are watched:
...
In addition to the [common input fields](https://docs.anthropic.com/en/docs/claude-code/hooks#common-input-fields), WorktreeCreate hooks receive the `name` field. This is a slug identifier for the new worktree, either specified by the user or auto-generated (for example, `bold-oak-a3f2`).
...
In addition to the [common input fields](https://docs.anthropic.com/en/docs/claude-code/hooks#common-input-fields), WorktreeRemove hooks receive the `worktree_path` field, which is the absolute path to the worktree being removed.
...
In addition to the [common input fields](https://docs.anthropic.com/en/docs/claude-code/hooks#common-input-fields), PreCompact hooks receive `trigger` and `custom_instructions`. For `manual`, `custom_instructions` contains what the user passes into `/compact`. For `auto`, `custom_instructions` is empty.
...
In addition to the [common input fields](https://docs.anthropic.com/en/docs/claude-code/hooks#common-input-fields), PostCompact hooks receive `trigger` and `compact_summary`. The `compact_summary` field contains the conversation summary generated by the compact operation.
...
In addition to the [common input fields](https://docs.anthropic.com/en/docs/claude-code/hooks#common-input-fields), SessionEnd hooks receive a `reason` field indicating why the session ended. See the [reason table](https://docs.anthropic.com/en/docs/claude-code/hooks#sessionend) above for all values.
...
In addition to the [common input fields](https://docs.anthropic.com/en/docs/claude-code/hooks#common-input-fields), Elicitation hooks receive `mcp_server_name`, `message`, and optional `mode`, `url`, `elicitation_id`, and `requested_schema` fields.For form-mode elicitation (the most common case):
...
In addition to the [common input fields](https://docs.anthropic.com/en/docs/claude-code/hooks#common-input-fields), ElicitationResult hooks receive `mcp_server_name`, `action`, and optional `mode`, `elicitation_id`, and `content` fields.
...
| `continueOnBlock` | no | When the prompt returns `ok: false`, feed the reason back to Claude and continue the turn instead of stopping. Default: `false`. Implemented as `continue: true` on the resulting `decision: "block"`. See [Response schema](https://docs.anthropic.com/en/docs/claude-code/hooks#response-schema) for per-event behavior |
...
*   `PermissionRequest`: `ok: false` has no effect. To deny an approval from a hook, use a [command hook](https://docs.anthropic.com/en/docs/claude-code/hooks#command-hook-fields) returning `hookSpecificOutput.decision.behavior: "deny"`
*   `PermissionDenied`: `ok: false` has no effect because the denial already happened. The only output this event reads is `hookSpecificOutput.retry`, which prompt and agent hooks cannot set — they run on this event, but their output is discarded. Use a [command hook](https://docs.anthropic.com/en/docs/claude-code/hooks#command-hook-fields) to return `retry`

If you need finer control on any event, use a [command hook](https://docs.anthropic.com/en/docs/claude-code/hooks#command-hook-fields) with the per-event fields described in [Decision control](https://docs.anthropic.com/en/docs/claude-code/hooks#decision-control).
...
Set `type` to `"agent"` and provide a `prompt` string. The configuration fields are the same as [prompt hooks](https://docs.anthropic.com/en/docs/claude-code/hooks#prompt-hook-configuration), with a longer default timeout:
