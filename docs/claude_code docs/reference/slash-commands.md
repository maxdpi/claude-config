...
Claude Code skills follow the [Agent Skills](https://agentskills.io/) open standard, which works across multiple AI tools. Claude Code extends the standard with additional features like [invocation control](https://docs.anthropic.com/en/docs/claude-code/slash-commands#control-who-invokes-a-skill), [subagent execution](https://docs.anthropic.com/en/docs/claude-code/slash-commands#run-skills-in-a-subagent), and [dynamic context injection](https://docs.anthropic.com/en/docs/claude-code/slash-commands#inject-dynamic-context).
...
Your `SKILL.md` can contain anything, but thinking through how you want the skill invoked (by you, by Claude, or both) and where you want it to run (inline or in a subagent) helps guide what to include. For complex skills, you can also [add supporting files](https://docs.anthropic.com/en/docs/claude-code/slash-commands#add-supporting-files) to keep the main skill focused.Keep the body itself concise. Once a skill loads, its content [stays in context across turns](https://docs.anthropic.com/en/docs/claude-code/slash-commands#skill-content-lifecycle), so every line is a recurring token cost. State what to do rather than narrating how or why, and apply the same conciseness test you would for [CLAUDE.md content](https://code.claude.com/docs/en/best-practices#write-an-effective-claude-md).
...
| --- | --- | --- |
| `name` | No | Display name shown in skill listings. Defaults to the directory name. See [How a skill gets its command name](https://docs.anthropic.com/en/docs/claude-code/slash-commands#how-a-skill-gets-its-command-name) for how this differs from the name you type to invoke the skill. |
...
| `arguments` | No | Named positional arguments for [`$name` substitution](https://docs.anthropic.com/en/docs/claude-code/slash-commands#available-string-substitutions) in the skill content. Accepts a space-separated string or a YAML list. Names map to argument positions in order. |
...
| `$name` | Named argument declared in the [`arguments`](https://docs.anthropic.com/en/docs/claude-code/slash-commands#frontmatter-reference) frontmatter list. Names map to positions in order, so with `arguments: [issue, branch]` the placeholder `$issue` expands to the first argument and `$branch` to the second. |
...
Save this to `~/.claude/skills/codebase-visualizer/SKILL.md`. The description tells Claude when to activate this Skill, and the instructions tell Claude to run the bundled script. The script path uses [`${CLAUDE_SKILL_DIR}`](https://docs.anthropic.com/en/docs/claude-code/slash-commands#available-string-substitutions) so it resolves correctly whether the skill is installed at the personal, project, or plugin level:
