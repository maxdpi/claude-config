...
URL Source: https://docs.anthropic.com/en/docs/claude-code/settings
...
| `allowedHttpHookUrls` | Allowlist of URL patterns that HTTP hooks may target. Supports `*` as a wildcard. When set, hooks with non-matching URLs are blocked. Undefined = no restriction, empty array = block all HTTP hooks. Arrays merge across settings sources. See [Hook configuration](https://docs.anthropic.com/en/docs/claude-code/settings#hook-configuration) | `["https://hooks.example.com/*"]` |
...
| `allowManagedHooksOnly` | (Managed settings only) Only managed hooks, SDK hooks, and hooks from plugins force-enabled in managed settings `enabledPlugins` are loaded. User, project, and all other plugin hooks are blocked. See [Hook configuration](https://docs.anthropic.com/en/docs/claude-code/settings#hook-configuration) | `true` |
...
| `attribution` | Customize attribution for git commits and pull requests. See [Attribution settings](https://docs.anthropic.com/en/docs/claude-code/settings#attribution-settings) | `{"commit": "🤖 Generated with Claude Code", "pr": ""}` |
...
| `httpHookAllowedEnvVars` | Allowlist of environment variable names HTTP hooks may interpolate into headers. When set, each hook’s effective `allowedEnvVars` is the intersection with this list. Undefined = no restriction. Arrays merge across settings sources. See [Hook configuration](https://docs.anthropic.com/en/docs/claude-code/settings#hook-configuration) | `["MY_TOKEN", "HOOK_SECRET"]` |
...
| `maxSkillDescriptionChars` | Per-skill character cap on the combined `description` and `when_to_use` text in the [skill listing](https://code.claude.com/docs/en/skills#skill-descriptions-are-cut-short) Claude sees each turn (default: `1536`). Text longer than this is truncated. Raise to keep long descriptions intact at the cost of more context per turn; lower to fit more skills under [`skillListingBudgetFraction`](https://docs.anthropic.com/en/docs/claude-code/settings#available-settings). Requires Claude Code v2.1.105 or later | `2048` |
...
| --- | --- | --- |
| `allow` | Array of permission rules to allow tool use. Tool-name globs are supported only in the tool position after a literal `mcp__<server>__` prefix, such as `mcp__github__get_*`; the server segment must be glob-free. See [Permission rule syntax](https://docs.anthropic.com/en/docs/claude-code/settings#permission-rule-syntax) below for pattern matching details | `[ "Bash(git diff *)" ]` |
| `ask` | Array of permission rules to ask for confirmation upon tool use. See [Permission rule syntax](https://docs.anthropic.com/en/docs/claude-code/settings#permission-rule-syntax) below | `[ "Bash(git push *)" ]` |
| `deny` | Array of permission rules to deny tool use. Use this to exclude sensitive files from Claude Code access. Tool names accept glob patterns: `"*"` denies every tool and `"mcp__*"` denies all MCP tools. See [Permission rule syntax](https://docs.anthropic.com/en/docs/claude-code/settings#permission-rule-syntax) and [Bash permission limitations](https://code.claude.com/docs/en/permissions#tool-specific-permission-rules) | `[ "WebFetch", "Bash(curl *)", "Read(./.env)", "Read(./secrets/**)" ]` |
...
These settings control which hooks are allowed to run and what HTTP hooks can access. The `allowManagedHooksOnly` setting can only be configured in [managed settings](https://docs.anthropic.com/en/docs/claude-code/settings#settings-files). The URL and env var allowlists can be set at any settings level and merge across sources.**Behavior when `allowManagedHooksOnly` is `true`:**
...
1.   **Managed settings** ([server-managed](https://code.claude.com/docs/en/server-managed-settings), [MDM/OS-level policies](https://docs.anthropic.com/en/docs/claude-code/settings#configuration-scopes), or [managed settings](https://code.claude.com/docs/en/settings#settings-files))
    ...
    *   Embedding hosts such as Claude Desktop can supply policy via the SDK `managedSettings` option. By default this is ignored when any managed-settings tier is present. Administrators can opt in by setting [`parentSettingsBehavior`](https://docs.anthropic.com/en/docs/claude-code/settings#available-settings) to `"merge"`. The embedder’s values are filtered so they can tighten managed policy but not loosen it.
