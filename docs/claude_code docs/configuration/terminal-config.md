...
URL Source: https://docs.anthropic.com/en/docs/claude-code/terminal-config
...
*   [Shift+Enter submits instead of inserting a newline](https://docs.anthropic.com/en/docs/claude-code/terminal-config#enter-multiline-prompts)
*   [Option-key shortcuts do nothing on macOS](https://docs.anthropic.com/en/docs/claude-code/terminal-config#enable-option-key-shortcuts-on-macos)
*   [No sound or alert when Claude finishes](https://docs.anthropic.com/en/docs/claude-code/terminal-config#get-a-terminal-bell-or-notification)
*   [You run Claude Code inside tmux](https://docs.anthropic.com/en/docs/claude-code/terminal-config#configure-tmux)
*   [Display flickers or scrollback jumps](https://docs.anthropic.com/en/docs/claude-code/terminal-config#switch-to-fullscreen-rendering)
*   [You want Vim keys in the prompt](https://docs.anthropic.com/en/docs/claude-code/terminal-config#edit-prompts-with-vim-keybindings)
...
When Claude finishes a task or pauses for a permission prompt, it fires a notification event. Surfacing this as a terminal bell or desktop notification lets you switch to other work while a long task runs.By default Claude Code sends a desktop notification only in Ghostty, Kitty, and iTerm2. In other terminals, set [`preferredNotifChannel`](https://code.claude.com/docs/en/settings#available-settings) to `"terminal_bell"` to ring the terminal bell instead, or configure a [Notification hook](https://docs.anthropic.com/en/docs/claude-code/terminal-config#play-a-sound-with-a-notification-hook) for a custom sound or command.The desktop notification reaches your local machine over SSH, so a remote session can still alert you. Ghostty and Kitty forward it to your OS notification center without further setup. iTerm2 requires you to enable forwarding:

1

2

If notifications still do not appear, confirm that your terminal application has notification permission in your OS settings, and if you are running inside tmux, [enable passthrough](https://docs.anthropic.com/en/docs/claude-code/terminal-config#configure-tmux).
