...
URL Source: https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python
...
Creates a new session for each interaction with Claude Code by default. Returns an async iterator that yields messages as they arrive. Each call to `query()` starts fresh with no memory of previous interactions unless you pass `continue_conversation=True` or `resume` in [`ClaudeAgentOptions`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#claudeagentoptions). See [Sessions](https://code.claude.com/docs/en/agent-sdk/sessions).
...
| `annotations` | [`ToolAnnotations`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#toolannotations)`| None` | Optional MCP tool annotations providing behavioral hints to clients |
...
| `tag` | `str | None` | User-set session tag (see [`tag_session()`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#tag_session)) |
...
Returns [`SDKSessionInfo`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#return-type-sdksessioninfo), or `None` if the session is not found.
...
Rename the most recent session so it’s easier to find later. The new title appears in [`SDKSessionInfo.custom_title`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#return-type-sdksessioninfo) on subsequent reads.
...
| `get_mcp_status()` | Get the status of all configured MCP servers. Returns [`McpStatusResponse`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#mcpstatusresponse) |
...
| `stop_task(task_id)` | Stop a running background task. A [`TaskNotificationMessage`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#tasknotificationmessage) with status `"stopped"` follows in the message stream |
...
| `betas` | `list[SdkBeta]` | `[]` | Beta features to enable. See [`SdkBeta`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#sdkbeta) for available options |
...
| `can_use_tool` | [`CanUseTool`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#canusetool)`| None` | `None` | Tool permission callback function. See [Permission types](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#canusetool) for details |
| `hooks` | `dict[HookEvent, list[HookMatcher]] | None` | `None` | Hook configurations for intercepting events |
| `user` | `str | None` | `None` | User identifier |
| `include_partial_messages` | `bool` | `False` | Include partial message streaming events. When enabled, [`StreamEvent`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#streamevent) messages are yielded |
...
| `sandbox` | [`SandboxSettings`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#sandboxsettings)`| None` | `None` | Configure sandbox behavior programmatically. See [Sandbox settings](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#sandboxsettings) for details |
...
| `thinking` | [`ThinkingConfig`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#thinkingconfig)`| None` | `None` | Controls extended thinking behavior. Takes precedence over `max_thinking_tokens` |
| `effort` | [`EffortLevel`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#effortlevel)`| None` | `None` | Effort level for thinking depth. See [adjust the effort level](https://code.claude.com/docs/en/model-config#adjust-effort-level) |
...
| `effort` | No | Reasoning effort level for this agent. Accepts a named level or an integer. See [`EffortLevel`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#effortlevel) |
| `permissionMode` | No | Permission mode for tool execution within this agent. See [`PermissionMode`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#permissionmode) |
...
The configuration of an MCP server as reported by [`get_mcp_status()`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#methods). This is the union of all [`McpServerConfig`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#mcpserverconfig) transport variants plus an output-only `claudeai-proxy` variant for servers proxied through claude.ai.
...
`McpSdkServerConfigStatus` is the serializable form of [`McpSdkServerConfig`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#mcpsdkserverconfig) with only `type` (`"sdk"`) and `name` (`str`) fields; the in-process `instance` is omitted. `McpClaudeAIProxyServerConfig` has `type` (`"claudeai-proxy"`), `url` (`str`), and `id` (`str`) fields.

### `McpStatusResponse`

Response from [`ClaudeSDKClient.get_mcp_status()`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#methods). Wraps the list of server statuses under the `mcpServers` key.
...
Status of a connected MCP server, contained in [`McpStatusResponse`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#mcpstatusresponse).
...
| `config` | [`McpServerStatusConfig`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#mcpserverstatusconfig) (optional) | Server configuration. Same shape as [`McpServerConfig`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#mcpserverconfig) (stdio, SSE, HTTP, or SDK), plus a `claudeai-proxy` variant for servers connected through claude.ai |
...
| `error` | [`AssistantMessageError`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#assistantmessageerror)`| None` | Error type if the response encountered an error |
| `usage` | `dict[str, Any] | None` | Per-message token usage (same keys as [`ResultMessage.usage`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#resultmessage)) |
...
The `model_usage` dict maps model names to per-model usage. The inner dict keys use camelCase because the value is passed through unmodified from the underlying CLI process, matching the TypeScript [`ModelUsage`](https://code.claude.com/docs/en/agent-sdk/typescript#modelusage) type:
...
| Field | Type | Description |
| --- | --- | --- |
| `rate_limit_info` | [`RateLimitInfo`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#ratelimitinfo) | Current rate limit state |
| `uuid` | `str` | Unique event identifier |
| `session_id` | `str` | Session identifier |

### `RateLimitInfo`

Rate limit state carried by [`RateLimitEvent`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#ratelimitevent).
...
Emitted when a background task starts. A background task is anything tracked outside the main turn: a backgrounded Bash command, a [Monitor](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#monitor) watch, a subagent spawned via the Agent tool, or a remote agent. The `task_type` field tells you which. This naming is unrelated to the `Task`-to-`Agent` tool rename.
...
*   `input`: Strongly-typed hook input with discriminated unions based on `hook_event_name` (see [`HookInput`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#hookinput))
*   `tool_use_id`: Optional tool use identifier (for tool-related hooks)
*   `context`: Hook context with additional information

Returns a [`HookJSONOutput`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#hookjsonoutput) that may contain:
...
**Tool name:**`AskUserQuestion`Asks the user clarifying questions during execution. See [Handle approvals and user input](https://code.claude.com/docs/en/agent-sdk/user-input#handle-clarifying-questions) for usage details.**Input:**
...
| `allowUnsandboxedCommands` | `bool` | `True` | Allow the model to request running commands outside the sandbox. When `True`, the model can set `dangerouslyDisableSandbox` in tool input, which falls back to the [permissions system](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#permissions-fallback-for-unsandboxed-commands) |
| `network` | [`SandboxNetworkConfig`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#sandboxnetworkconfig) | `None` | Network-specific sandbox configuration |
| `ignoreViolations` | [`SandboxIgnoreViolations`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#sandboxignoreviolations) | `None` | Configure which sandbox violations to ignore |
