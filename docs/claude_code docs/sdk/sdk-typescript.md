...
URL Source: https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript
...
| --- | --- | --- |
| `prompt` | `string | AsyncIterable<`[`SDKUserMessage`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#sdkusermessage)`>` | The input prompt as a string or async iterable for streaming mode |
| `options` | [`Options`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#options) | Optional configuration object (see Options type below) |

#### Returns

Returns a [`Query`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#query-object) object that extends `AsyncGenerator<`[`SDKMessage`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#sdkmessage)`, void>` with additional methods.

### `startup()`

Pre-warms the CLI subprocess by spawning it and completing the initialize handshake before a prompt is available. The returned [`WarmQuery`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#warmquery) handle accepts a prompt later and writes it to an already-ready process, so the first `query()` call resolves without paying subprocess spawn and initialization cost inline.
...
| Parameter | Type | Description |
| --- | --- | --- |
| `options` | [`Options`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#options) | Optional configuration object. Same as the `options` parameter to `query()` |
...
Returns a `Promise<`[`WarmQuery`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#warmquery)`>` that resolves once the subprocess has spawned and completed its initialize handshake.
...
| `handler` | `(args, extra) => Promise<`[`CallToolResult`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#calltoolresult)`>` | Async function that executes the tool logic |
| `extras` | `{ annotations?:`[`ToolAnnotations`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#toolannotations)`}` | Optional MCP tool annotations providing behavioral hints to clients |
...
| `options.tools` | `Array<SdkMcpToolDefinition>` | Array of tool definitions created with [`tool()`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#tool) |
...
| `tag` | `string | undefined` | User-set session tag (see [`tagSession()`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#tagsession)) |
...
Returns [`SDKSessionInfo`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#return-type-sdksessioninfo), or `undefined` if the session is not found.
...
| `options.settingSources` | [`SettingSource`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#settingsource)`[]` | All sources | Which filesystem sources to load. Pass `[]` to skip user, project, and local settings. Managed policy settings load in all cases |
| `options.managedSettings` | `Settings` | `undefined` | Restrictive policy-tier settings supplied by the embedding host. Dropped by default when an admin-deployed managed tier is present; merged under that tier when [`parentSettingsBehavior`](https://code.claude.com/docs/en/settings#available-settings) is `"merge"`. Non-restrictive keys such as `model` are silently dropped so this option can tighten managed policy but not loosen it |
...
| `agentProgressSummaries` | `boolean` | `false` | When `true`, generate one-line progress summaries for subagents and forward them on [`task_progress`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#sdktaskprogressmessage) events via the `summary` field. Applies to foreground and background subagents |
...
| `betas` | [`SdkBeta`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#sdkbeta)`[]` | `[]` | Enable beta features |
| `canUseTool` | [`CanUseTool`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#canusetool) | `undefined` | Custom permission function for tool usage |
...
| `env` | `Record<string, string | undefined>` | `process.env` | Environment variables. When set, this replaces the subprocess environment instead of merging with `process.env`, so pass `{ ...process.env, YOUR_VAR: 'value' }` to keep inherited variables like `PATH`. See [Handle slow or stalled API responses](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#handle-slow-or-stalled-api-responses) for an example of this pattern, and [Environment variables](https://code.claude.com/docs/en/env-vars) for variables the underlying CLI reads. Set `CLAUDE_AGENT_SDK_CLIENT_APP` to identify your app in the User-Agent header |
...
| `hooks` | `Partial<Record<`[`HookEvent`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#hookevent)`,`[`HookCallbackMatcher`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#hookcallbackmatcher)`[]>>` | `{}` | Hook callbacks for events |
| `includeHookEvents` | `boolean` | `false` | Include hook lifecycle events in the message stream as [`SDKHookStartedMessage`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#sdkhookstartedmessage), [`SDKHookProgressMessage`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#sdkhookprogressmessage), and [`SDKHookResponseMessage`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#sdkhookresponsemessage) |
...
| `permissionMode` | [`PermissionMode`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#permissionmode) | `'default'` | Permission mode for the session |
...
| `plugins` | [`SdkPluginConfig`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#sdkpluginconfig)`[]` | `[]` | Load custom plugins from local paths. See [Plugins](https://code.claude.com/docs/en/agent-sdk/plugins) for details |
...
| `sandbox` | [`SandboxSettings`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#sandboxsettings) | `undefined` | Configure sandbox behavior programmatically. See [Sandbox settings](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#sandboxsettings) for details |
| `sessionId` | `string` | Auto-generated | Use a specific UUID for the session instead of auto-generating one |
| `sessionStore` | [`SessionStore`](https://code.claude.com/docs/en/agent-sdk/session-storage#the-sessionstore-interface) | `undefined` | Mirror session transcripts to an external backend so any host can resume them. See [Persist sessions to external storage](https://code.claude.com/docs/en/agent-sdk/session-storage) |
| `sessionStoreFlush` | `'batched' | 'eager'` | `'batched'` | _Alpha._ Flush mode for `sessionStore`. Ignored when `sessionStore` is not set |
| `settings` | `string | Settings` | `undefined` | Inline [settings](https://code.claude.com/docs/en/settings) object or path to a settings file. Populates the flag-settings layer in the [precedence order](https://code.claude.com/docs/en/settings#settings-precedence). Change at runtime with [`applyFlagSettings()`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#applyflagsettings) |
| `settingSources` | [`SettingSource`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#settingsource)`[]` | CLI defaults (all sources) | Control which filesystem settings to load. Pass `[]` to disable user, project, and local settings. Managed policy settings load regardless. See [Use Claude Code features](https://code.claude.com/docs/en/agent-sdk/claude-code-features#what-settingsources-does-not-control) |
...
| `thinking` | [`ThinkingConfig`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#thinkingconfig) | `{ type: 'adaptive' }` for supported models | Controls Claude’s thinking/reasoning behavior. See [`ThinkingConfig`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#thinkingconfig) for options |
| `title` | `string` | `undefined` | Display title for the session. When resuming via `resume` or `continue`, the resumed session’s persisted title takes precedence; use [`renameSession()`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#renamesession) to retitle an existing session |
...
| `toolConfig` | [`ToolConfig`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#toolconfig) | `undefined` | Configuration for built-in tool behavior. See [`ToolConfig`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#toolconfig) for details |
...
| `applyFlagSettings(settings)` | Merges settings into the session’s flag settings layer at runtime (only available in streaming input mode). See [`applyFlagSettings()`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#applyflagsettings) |
...
| `supportedAgents()` | Returns available subagents as [`AgentInfo`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#agentinfo)`[]` |
...
Handle returned by [`startup()`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#startup). The subprocess is already spawned and initialized, so calling `query()` on this handle writes the prompt directly to a ready process with no startup latency.
...
| Method | Description |
| --- | --- |
| `query(prompt)` | Send a prompt to the pre-warmed subprocess and return a [`Query`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#query-object). Can only be called once per `WarmQuery` |
...
| `permissionMode` | No | Permission mode for tool execution within this agent. See [`PermissionMode`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#permissionmode) |
...
| `suggestions` | [`PermissionUpdate`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#permissionupdate)`[]` | Suggested permission updates so the user is not prompted again for this tool. Bash prompts include a suggestion with the `localSettings`[destination](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#permissionupdatedestination), so returning it in `updatedPermissions` writes the rule to `.claude/settings.local.json` and persists across sessions. |
...
The `origin` field forwards the [`SDKMessageOrigin`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#sdkmessageorigin) of the user message that triggered this result. When a background task finishes and the SDK injects a synthetic follow-up turn, the resulting `SDKResultMessage` carries `origin: { kind: "task-notification" }`. Check this field to distinguish results that answer your prompt from results emitted for background-task follow-ups, so you can route or suppress the latter. The field is absent for results emitted before any user turn, such as startup errors.When a `PreToolUse` hook returns `permissionDecision: "defer"`, the result has `stop_reason: "tool_deferred"` and `deferred_tool_use` carries the pending tool’s `id`, `name`, and `input`. Read this field to surface the request in your own UI, then resume with the same `session_id` to continue. See [Defer a tool call for later](https://code.claude.com/docs/en/hooks#defer-a-tool-call-for-later) for the full round trip.
...
| `task-notification` | Synthetic turn injected after a background task finished. See [`SDKTaskNotificationMessage`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#sdktasknotificationmessage). |
...
See [`McpServerConfig`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#mcpserverconfig) for details on each transport type.
...
A version of [`Usage`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#usage) with all nullable fields made non-nullable.
...
Notification when a background task completes, fails, or is stopped. Background tasks include `run_in_background` Bash commands, [Monitor](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#monitor) watches, and background subagents.
...
Emitted when a background task begins. The `task_type` field is `"local_bash"` for background Bash commands and [Monitor](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#monitor) watches, `"local_agent"` for subagents, or `"remote_agent"`.
...
Emitted periodically while a subagent or background task is running. The `summary` field is populated only when [`agentProgressSummaries`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#options) is enabled.
...
| `allowUnsandboxedCommands` | `boolean` | `true` | Allow the model to request running commands outside the sandbox. When `true`, the model can set `dangerouslyDisableSandbox` in tool input, which falls back to the [permissions system](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#permissions-fallback-for-unsandboxed-commands) |
| `network` | [`SandboxNetworkConfig`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#sandboxnetworkconfig) | `undefined` | Network-specific sandbox configuration |
| `filesystem` | [`SandboxFilesystemConfig`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#sandboxfilesystemconfig) | `undefined` | Filesystem-specific sandbox configuration for read/write restrictions |
...
Network-specific configuration for sandbox mode. These settings apply to sandboxed Bash commands when `enabled` is `true` in the parent [`SandboxSettings`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-typescript#sandboxsettings). They do not restrict the WebFetch tool, which uses [permission rules](https://code.claude.com/docs/en/permissions#webfetch) instead.
...
| `allowManagedDomainsOnly` | `boolean` | `false` | Managed-settings only. When set in [managed settings](https://code.claude.com/docs/en/permissions#managed-settings), only `allowedDomains` entries from managed settings are honored and entries from user, project, or local settings are ignored. Has no effect when set via SDK options |
