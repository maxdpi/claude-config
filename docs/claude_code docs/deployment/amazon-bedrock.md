...
Claude Code supports automatic credential refresh for AWS SSO and corporate identity providers. Add these settings to your Claude Code settings file (see [Settings](https://code.claude.com/docs/en/settings) for file locations).These two settings have different trigger conditions:
...
*   The WebSearch tool is not available on Bedrock. See [WebSearch tool behavior](https://code.claude.com/docs/en/tools-reference#websearch-tool-behavior).
