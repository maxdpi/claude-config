...
URL Source: https://docs.anthropic.com/en/docs/claude-code/hooks-guide

Markdown Content:
Hooks are user-defined shell commands that execute at specific points in Claude Code’s lifecycle. They provide deterministic control over Claude Code’s behavior, ensuring certain actions always happen rather than relying on the LLM to choose to run them. Use hooks to enforce project rules, automate repetitive tasks, and integrate Claude Code with your existing tools.For decisions that require judgment rather than deterministic rules, you can also use [prompt-based hooks](https://docs.anthropic.com/en/docs/claude-code/hooks-guide#prompt-based-hooks) or [agent-based hooks](https://docs.anthropic.com/en/docs/claude-code/hooks-guide#agent-based-hooks) that use a Claude model to evaluate conditions.For other ways to extend Claude Code, see [skills](https://code.claude.com/docs/en/skills) for giving Claude additional instructions and executable commands, [subagents](https://code.claude.com/docs/en/sub-agents) for running tasks in isolated contexts, and [plugins](https://code.claude.com/docs/en/plugins) for packaging extensions to share across projects.

## Set up your first hook

To create a hook, add a `hooks` block to a [settings file](https://docs.anthropic.com/en/docs/claude-code/hooks-guide#configure-hook-location). This walkthrough creates a desktop notification hook, so you get alerted whenever Claude is waiting for your input instead of watching the terminal.
...
Hooks let you run code at key points in Claude Code’s lifecycle: format files after edits, block commands before they execute, send notifications when Claude needs input, inject context at session start, and more. For the full list of hook events, see the [Hooks reference](https://code.claude.com/docs/en/hooks#hook-lifecycle).Each example includes a ready-to-use configuration block that you add to a [settings file](https://docs.anthropic.com/en/docs/claude-code/hooks-guide#configure-hook-location). The most common patterns:

*   [Get notified when Claude needs input](https://docs.anthropic.com/en/docs/claude-code/hooks-guide#get-notified-when-claude-needs-input)
*   [Auto-format code after edits](https://docs.anthropic.com/en/docs/claude-code/hooks-guide#auto-format-code-after-edits)
*   [Block edits to protected files](https://docs.anthropic.com/en/docs/claude-code/hooks-guide#block-edits-to-protected-files)
*   [Re-inject context after compaction](https://docs.anthropic.com/en/docs/claude-code/hooks-guide#re-inject-context-after-compaction)
*   [Audit configuration changes](https://docs.anthropic.com/en/docs/claude-code/hooks-guide#audit-configuration-changes)
*   [Reload environment when directory or files change](https://docs.anthropic.com/en/docs/claude-code/hooks-guide#reload-environment-when-directory-or-files-change)
*   [Auto-approve specific permission prompts](https://docs.anthropic.com/en/docs/claude-code/hooks-guide#auto-approve-specific-permission-prompts)
...
*   `"type": "http"`: POST event data to a URL. See [HTTP hooks](https://docs.anthropic.com/en/docs/claude-code/hooks-guide#http-hooks).
...
*   `"type": "prompt"`: single-turn LLM evaluation. See [Prompt-based hooks](https://docs.anthropic.com/en/docs/claude-code/hooks-guide#prompt-based-hooks).
*   `"type": "agent"`: multi-turn verification with tool access. Agent hooks are experimental and may change. See [Agent-based hooks](https://docs.anthropic.com/en/docs/claude-code/hooks-guide#agent-based-hooks).
